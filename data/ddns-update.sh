...
for url_ip in "${PRIMARY_IP_SOURCES[@]}"; do
    response=$(curl -6sSL --connect-timeout 2 --max-time 3 --user-agent "${CURL_USER_AGENT}" "$url_ip" 2>/dev/null)
    if [[ "$response" =~ ^[0-9a-fA-F:]+$ ]]; then
        export IP_SOURCE="$url_ip"
        break
    fi
done
IP=$(curl -6sSL --user-agent "${CURL_USER_AGENT}" "$IP_SOURCE" 2>/dev/null)
UPDIP=$(cat $PFAD/updip.txt)
sleep 1
...
if [ "$IP" == "$UPDIP" ]; then
    ...
    sleep 1
    UPDATE_IP=$(curl -6sSL --user-agent "${CURL_USER_AGENT}" "https://ipv64.net/update.php?key=${DOMAIN_KEY}&domain=${DOMAIN_IPV64}&ip=${IP}&output=min" 2>/dev/null)
    if [[ "$UPDATE_IP" =~ (nochg|good|ok) ]] ; then
        ...
        echo "$DATUM  FEHLER !!!  - UPDATE IP=$IP WURDE NICHT AN
